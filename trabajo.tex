...
La imagen \ref{fig:EC_add} muestra la idea geométrica de la operación de suma

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{ec_add}
	\caption{Suma de la curva elíptica $\mathbb{R}$ [https://sinhvientot.net/elliptic-curve-cryptography/]}
	\label{fig:EC_add}
\end{figure}
...
Lo más habitual es que el cuerpo sobre el que estamos trabajando sea un cuerpo finito $ \mathbb{F}_p $ con $ p $ un primo.En ese caso, la curva vendría dada por:
\[E = \{(x,y) \in K\times K : y^2 \equiv x^3 + ax +b \bmod p,\ 4a^3+27b \not\equiv 0 \bmod p\} \cup \{\infty\}.
\] 
Veamos cómo se visualizan las curvas en estos casos.

\begin{lstlisting}[label={cod:ej5}, caption={Curva elíptica $y^2 + y=x^3-x $ sobre $ \mathbb{F}_{p}$.}, morekeywords={sage}]
...
(REFERENCIA: https://es.bitcoin.it/wiki/Secp256k1)
\item NIST P-256: es la que trae \textit{OpenSSL} por defecto y he existe una gran cantidad de métodos para optimizar su uso. \\
(REFERENCIA: https://tools.ietf.org/html/rfc7748\#page-4) \\
REFERENCIA: https://es.wikipedia.org/wiki/Curve25519
...
(REFERENCIA: https://es.wikipedia.org/wiki/Curva448)
\end{enumerate}
