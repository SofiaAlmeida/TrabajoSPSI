...
Ahora entra en juego Bob que quiere validar la firma, $(r,s)$, a partir del mensaje recibido con valor \textit{hash} \textit{z} y de la clave pública de Alice \textit{$H_A$}.

\begin{enumerate}
	\item Calculamos $u_1 \equiv$ \textit{$s^{-1}$ z} mod\textit{n}.
	\item Calculamos $u_2 \equiv$ \textit{$s^{-1}$ r} mod\textit{n}.
	\item Calculamos el punto \textit{P} = \textit{$u_1$G} + \textit{$u_2$$H_A$}.	
\end{enumerate}
...
Veamos que esta definición de \textit{P} del punto 3 coincide con la definición original de \textit{P} = \textit{kG}.
...
Como el subgrupo cíclico de orden \textit{n} en el que estamos trabajando está generado por \textit{G} no hace falta poner ``mod \textit{n}'' si multiplicamos por \textit{G}.
...
Es importante el valor que escojamos de \textit{k} el entero aleatorio utilizado para firmar. Si además siempre firmamos con el mismo valor de \textit{k} y con los mismos valores aleatorios, para los atacantes será más fácil encontrar la clave privada.

Para descubrir la clave privada \textit{$d_S$} necesitamos calcular \textit{k} a partir de los valores \textit{hash} \textit{$z_1$} y \textit{$z_2$} y de las correspondientes firmas (\textit{$r_1$}, \textit{$s_1$}) y (\textit{$r_2$}, \textit{$s_2$}).
